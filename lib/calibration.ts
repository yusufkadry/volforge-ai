...
export async function calibrate() {
  ...
  const status: CalibrationSnapshot["status"] = sampleSize < 5 ? "warming" : realizedPnl < 0 && divergence < -0.75 ? "degraded" : "calibrated";
  const snapshot: CalibrationSnapshot = {
    ...
    predicted_win_rate: predictedWinRate, realized_win_rate: realizedWinRate, status,
    report: { divergence, closed_intents: intents.map((intent) => ({ id: intent.id, underlying: intent.underlying, entry_debit: intent.entry_debit, exit_debit: intent.current_debit, quantity: intent.quantity })) },
  };
