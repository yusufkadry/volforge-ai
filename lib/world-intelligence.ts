...
export async function assessWorldIntelligence(candidate: Candidate): Promise<WorldEvidence> {
  try {
    ...
    const classified = (response.news ?? []).flatMap((article) => {
      ...
      const scope: WorldArticle["scope"] = symbols.includes(candidate.underlying) ? "issuer" : "market";
    ...
    const directionalScore = articles.reduce((total, article) => total + article.direction * article.reliability * Math.exp(-article.ageHours / 36), 0);
    const eventRiskScore = articles.reduce((total, article) => total + (article.impact === "high" ? 1 : article.impact === "medium" ? 0.4 : 0.1) * article.reliability * Math.exp(-article.ageHours / 24), 0);
    const strategyDirection = candidate.contractType === "call" ? 1 : -1;
    const contradiction = directionalScore * strategyDirection < -0.8;
    const unmodeledJump = articles.some((article) => article.impact === "high" && article.ageHours <= 24 && ["earnings", "macro", "regulatory", "security", "geopolitical"].includes(article.category));
    const verdict: WorldEvidence["verdict"] = contradiction || unmodeledJump ? "veto" : directionalScore * strategyDirection >= 1.2 && eventRiskScore < 1.5 ? "approve" : "abstain";
    ...
    const tags = [...new Set(articles.filter((article) => article.category !== "other").map((article) => article.category))];
    const rationale = verdict === "veto"
      ? unmodeledJump
        ? `A high-impact ${articles.find((article) => article.impact === "high" && article.ageHours <= 24)?.category ?? "event"} catalyst falls outside the calibrated return model; entry vetoed until repriced.`
        : `Reliability- and recency-weighted news flow contradicts the ${candidate.contractType} thesis (${directionalScore.toFixed(2)} score).`
      : verdict === "approve"
        ? `Recency-weighted, source-qualified evidence aligns with the ${candidate.contractType} thesis without an unmodeled jump event.`
        : `${articles.length} deduplicated issuer and market articles were classified by source, age, event type, and direction; evidence remains non-decisive.`;
    const evidence = { candidate: candidate.optionSymbol, articles, directionalScore, eventRiskScore, verdict, rationale, pagination: response.meta };
    return { engine: "Event Intelligence", verdict, confidence, expiresAt: new Date(Date.now() + 30 * 60_000).toISOString(), evidenceHash: createHash("sha256").update(JSON.stringify(evidence)).digest("hex"), headlineCount: articles.length, eventTags: tags, eventRiskScore, directionalScore, rationale, articles };
  } catch (error) {
