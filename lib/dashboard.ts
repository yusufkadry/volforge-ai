...
export async function getDashboardSnapshot(): Promise<DashboardSnapshot> {
  const settled = await Promise.allSettled([
    alpaca.account(), alpaca.positions(), alpaca.orders(), alpaca.clock(), journal.decisions(), journal.settings(), journal.research(), journal.shadowPositions(), journal.latestRiskSnapshot(),
  ]);
  const [account, positions, orders, clock, decisions, settings, research, shadowPositions, riskSnapshot] = settled.map((result) => result.status === "fulfilled" ? result.value : null);
